...
def get_grievance(tracking_id: str):
    ...
    c.execute(
        "SELECT * FROM grievances WHERE tracking_id = ?",
        (tracking_id,),
    )
...
def update_status(tracking_id: str, status: str, note: str = ""):
    ...
    c.execute(
        "UPDATE grievances SET status = ?, updated_at = ? WHERE tracking_id = ?",
...
def get_history(tracking_id: str):
    ...
    c.execute(
        "SELECT status, note, timestamp FROM history "
...
KEYWORDS = {
    "pothole": [
        "pothole", "pot hole", "road damage", "broken road", "crack",
        "asphalt", "bump", "uneven road", "sinkhole"
    ],
    "garbage": [
        "garbage", "trash", "waste", "litter", "dump", "rubbish",
        "bin overflow", "dirty", "smell", "rotting", "debris"
    ],
    "water_leak": [
        "water leak", "leaking", "pipe burst", "water pipe", "tap",
        "flooding", "sewage", "overflow", "wet road", "leak"
    ],
    "streetlight": [
        "streetlight", "street light", "lamp", "bulb", "dark road",
        "no light", "pole", "flickering light", "broken light"
    ],
    "drainage": [
        "drain", "drainage", "clogged", "blocked drain", "manhole",
        "sewer", "gutter", "stagnant water"
    ],
    "stray_animal": [
        "stray dog", "stray cat", "cow", "animal", "monkey",
        "dog bite", "animal menace", "cattle"
    ],
    "tree_hazard": [
        "fallen tree", "tree branch", "tree", "branch",
        "uprooted", "hanging branch", "tree hazard"
    ],
}
...
def extract_photo_gps(path: Path):
    """Read GPS coordinates stored in a photo's EXIF metadata.

    This uses only metadata already embedded in the uploaded photo.
    It does not access the user's current GPS location.
    """
    try:
        ...
        gps_info = exif.get(34853)  # EXIF GPSInfo tag
        if not gps_info:
            return None, None
...
placeholder="e.g., Large pothole on MG Road near bus stop..."></textarea>

<label>📍 Location</label>

<div class="row">
<input id="location" type="text"
placeholder="Street / Area / Landmark (optional if photo has GPS)">
...
<p id="photoGpsStatus" class="muted" style="margin-top:8px">
If the uploaded photo contains GPS metadata, Theervu.AI can use the photo's captured location instead of your current location.
</p>
...
<span id="recordStatus" class="muted">
Not recording
</span>
</div>

<textarea id="voiceTranscript" rows="2"
...
<div class="row">
<input id="trackInput"
placeholder="Enter Tracking ID (e.g., GRV-20250101-A1B2C3)">
...
const photoGpsStatus = document.getElementById("photoGpsStatus");
...
document.getElementById("photo").addEventListener("change", (e) => {
    ...
    if (!photo) {
        photoGpsStatus.textContent =
            "If the uploaded photo contains GPS metadata, Theervu.AI can use the photo's captured location instead of your current location.";
        return;
    }

    photoGpsStatus.textContent =
        "📍 Photo selected. The server will check whether this image contains captured-location GPS metadata.";
});
...
form.addEventListener("submit", async (e) => {
    ...
    try {
        ...
        resultBody.innerHTML = `
            ...
            <div class="result-box">
                <div class="label">Captured / Reported Location</div>
                <div class="value">
    ...
    box.innerHTML = `
        ...
        ${
            ...
            ? `<p style="margin-top:10px">
                <a href="https://www.google.com/maps?q=${g.latitude},${g.longitude}"
                   target="_blank"
                   style="color:#a5b4fc">
                   📍 Open reported location in Google Maps
                </a>
               </p>`
            : ""
        }

        <h3 style="margin-top:16px;color:#c7d2fe">
            Timeline
        </h3>
...
async def create_grievance(
    ...
    # If the user has moved away after taking the photo, prefer
    # GPS metadata embedded in the photo over the phone's current GPS.
    photo_latitude = None
    photo_longitude = None

    if file_path and file_type == "image":
        photo_latitude, photo_longitude = extract_photo_gps(dest)
    ...
    if photo_latitude is not None and photo_longitude is not None:
        ...
        if not location.strip():
            location = (
                f"Photo captured location: "
                f"{latitude:.6f}, {longitude:.6f}"
            )
    elif not location.strip() and latitude is not None and longitude is not None:
        location = f"Current GPS: {latitude:.6f}, {longitude:.6f}"
